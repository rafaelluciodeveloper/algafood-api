...
public class ApiExceptionHandler extends ResponseEntityExceptionHandler {
	...
	private ResponseEntity<Object> handlerInvalidFormat(InvalidFormatException ex, HttpHeaders headers,
		...
		String detail = String.format(
				"A Propiedade '%s' recebeu o valor '%s', que e de um tipo inválido. Corrija e informe um valor compativel com o tipo %s.",
	...
	protected ResponseEntity<Object> handleExceptionInternal(Exception ex, Object body, HttpHeaders headers,
		...
		if (body == null) {
			body = Problem.builder().timestamp(LocalDateTime.now()).status(status.value())
					.title(status.getReasonPhrase()).userMessage(MSG_ERRO_GENERICA_USUARIO_FINAL).build();
